...
Copilot applies the sandbox to model-visible shell commands and local MCP/LSP subprocesses; its in-process built-in file and URL tools enforce the same filesystem and network policy. Waza ships Copilot CLI 1.0.80, the verified minimum for redirect and cross-origin URL enforcement. Model-backed prompt graders retain the task sandbox. Remote MCP servers and trusted post-execution program graders remain separate trust boundaries; program-grader commands run with host permissions. Copilot local sandboxing is in public preview; Windows currently requires a Windows Insiders build.
