...
| `schemaVersion` | string | ✗     | Public schema version for this artifact; defaults to the current version (`1.3`)        |
...
`schemaVersion` uses `MAJOR.MINOR` format. When omitted, Waza interprets it as the current version (`1.3`). Add it to new evals so future schema migrations are explicit. See [Schema Changes](/reference/schema-changes/) for the compatibility policy.
...
| `sandbox`           | object    | —                 | Opt into Copilot-native command isolation. Waza grants task-workspace read/write and declared-skill read-only access; risky capabilities default off. Requires `schemaVersion: "1.3"`. |

### Copilot-native sandbox

For `copilot-sdk` evaluations that expose shell or file tools, enable Copilot CLI's native OS sandbox:
...
Waza derives filesystem permissions from existing declarations: the fresh task workspace is read/write, while resolved `skill_directories` are read-only and executable through Copilot's native skill mechanism, so bundled scripts remain available without copying the skill. Fixtures and Git resources are copied or materialised into the task workspace. System temp, unrelated host paths, and sandbox-bypass requests are denied. Use optional `readonly_paths` or `readwrite_paths` only for declared host prerequisites that cannot be materialised in the workspace. Paths support `~` and environment-variable expansion and must resolve to existing absolute paths; references to unset or empty variables fail validation. Paths are canonicalised and cannot overlap the system temp root or receive conflicting read-only/read-write grants. Omitting the block or setting `enabled: false` sends no sandbox policy RPC and retains the existing process environment. Sandboxed Copilot CLI processes receive an allowlisted operational environment rather than arbitrary host variables, and credential-bearing proxy URLs are omitted.
...
Sandboxed evaluations share a Copilot CLI process only when their startup arguments, CLI path, allowlisted environment, and authentication token match. Changing the home directory, proxy, or token starts a separate client. The compatibility key fingerprints environment and authentication values rather than storing raw secrets.

Sandboxed suggestion analysis grants access and copies resources only from resolved skill directories containing `SKILL.md`, not their discovery parents or unrelated eval-directory files. If sandbox setup fails, Waza disconnects and deletes the affected session, including resumed sessions, and reports any cleanup failure. With `enabled: false`, optional sandbox paths are not resolved, and caching treats the block like an omitted sandbox policy.

Sandbox setup verifies the running CLI version through the SDK and requires 1.0.80 or newer for native URL enforcement. Older or unverifiable versions fail closed, including `COPILOT_CLI_PATH` overrides. Upgrade the overridden CLI or unset `COPILOT_CLI_PATH` to use Waza's bundled runtime.
...
`waza run` requires the lockfile and cached module contents to be present for remote refs. It fails closed on missing locks, missing cache entries, or digest mismatches.
