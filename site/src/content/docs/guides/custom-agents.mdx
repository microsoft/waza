...
This is enforced two ways: the Copilot SDK session is configured to only expose the declared tools where the SDK supports it, and a fail-closed permission handler denies any tool request the policy doesn't explicitly allow — including requests of a kind waza doesn't recognize. A denied attempt fails the run even if the agent goes on to produce an otherwise correct answer. The effective policy and every denial are recorded in `results.json` under each run's `session_digest` (`tool_policy_mode`, `tool_policy_denials`).

Tool-name matching is exact and case-insensitive, matching the `tool_constraint` grader's `allow_only` semantics — declaring one MCP tool does not implicitly allow every other tool on the same MCP server.
