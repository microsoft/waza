...
For MCP tools, declare the full `<server-key>-<tool-name>` identity, optionally prefixed with `mcp:` (for example `mcp:github-list_issues`). A bare tool name does not authorize a server or another server's tool. Explicit `mcp:` and `custom:` names retain their original case and source qualifier in the SDK filter. Hooks and transcripts expose tool names without source metadata, so source isolation relies on the SDK filter; name normalization alone is not a source boundary. Subagent launches must pass both the tool-name check (typically `task`) and the factory permission check using the factory's name, or `task` if unnamed. Permitting a launcher is not a separate sandbox for the subagent.
