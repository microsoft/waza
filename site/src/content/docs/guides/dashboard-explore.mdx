...
The landing page shows all evaluation runs at a glance. Seven KPI cards summarize your total runs, tasks, pass rate, token usage, premium-request credits, cost, and duration. Below the cards, a sortable table lists every run with its spec, model, pass rate, weighted score, task count, tokens, credits, cost, duration, and relative timestamp.

<ZoomImage src="../../images/explore/runs-overview.png" alt="Eval Runs overview showing KPI cards and sortable runs table" />

- **KPI cards** — Total Runs, Total Tasks, Pass Rate, Avg Tokens, Avg Credits, Avg Cost, and Avg Duration update in real time as results load
- **Sortable columns** — click any column header (Spec, Model, Tasks, Tokens, Credits, Cost, Duration, When) to sort ascending or descending
- **Credits** — premium-request count reported by the Copilot SDK (not dollars). Hover the column header or KPI label for a reminder
...
Switch to the **Prompts** tab to verify the exact resolved instruction captured for each task. JSON prompts are formatted for readability, and the copy action makes it easy to reuse the prompt while debugging an eval.

<ZoomImage src="../../images/explore/prompts-tab.png" alt="Run detail Prompts tab showing plain-text and formatted JSON task prompts with copy actions" />

- **Resolved content** — shows the raw prompt sent to the agent after file loading and template resolution
- **Format detection** — JSON objects and arrays are pretty-printed; other prompts remain plain text
- **Copy action** — copies the captured prompt and reports when browser clipboard access is unavailable
- **Legacy results** — artifacts created before prompt capture show an explicit unavailable state
...
- **Metrics comparison** — Pass Rate, Tokens, Credits, Cost, and Duration are compared with delta indicators (↑ increase in red, ↓ decrease in green)
...
The Trends page charts your evaluation metrics over time. Five line charts show Pass Rate, Tokens per Run, Cost per Run, Credits per Run, and Duration per Run across all runs, ordered chronologically.

<ZoomImage src="../../images/explore/trends.png" alt="Trends page with pass rate, tokens, cost, credits, and duration charts" />
...
- **Credits per Run** — track premium-request usage reported by the Copilot SDK
