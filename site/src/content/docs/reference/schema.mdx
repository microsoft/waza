...
schemaVersion: "1.3" # Optional: schema version; defaults to the current version
...
**Type:** string  
**Required:** no  
**Default:** current schema version (currently `1.3`)
...
**Type:** object
**Default:** omitted
**Requires:** `schemaVersion: "1.3"` or newer

Enables Copilot CLI's native OS sandbox for model-visible tools. Waza derives filesystem access from the fresh task workspace and resolved `skill_directories`. Declared skills remain read-only and are loaded through Copilot's native skill mechanism. Optional path lists are reserved for narrow host prerequisites that cannot be materialised in the workspace.
...
All optional capabilities default to `false`. Sandbox bypass requests are always rejected because Waza evaluations are non-interactive. Omitting the block or setting `enabled: false` sends no sandbox policy RPC. `readonly_paths` and `readwrite_paths` default to empty and accept absolute paths, `~`, and set environment-variable expansion for explicit host prerequisites. Paths must exist, are canonicalised, cannot overlap a denied temporary root, and cannot receive conflicting access grants. Sandboxed Copilot CLI processes receive an explicit allowlisted environment, with invalid temporary-directory values and credential-bearing proxy URLs omitted; GitHub authentication is passed explicitly through the SDK or loaded from persisted Copilot login. Omitting or disabling the sandbox retains the existing process environment. Copilot applies the OS sandbox to model-visible shell commands and local MCP/LSP subprocesses; the bundled Copilot CLI 1.0.80 enforces the configured filesystem and network policy for in-process built-in file and URL tools. Model-backed prompt graders retain the task sandbox. Remote MCP servers and trusted program graders remain separate trust boundaries; program-grader commands run with host permissions.
...
When `false`, Waza keeps Copilot SDK skill discovery enabled but does not add the target `<skill_context>` block or a synthetic `<available_skills>` summary. This is useful for trigger-precision evals that use `behavior` or `skill_invocation` graders to measure whether the agent actually invokes the `skill` tool. `disabled_skills: ["*"]` takes precedence and disables all skill loading.
