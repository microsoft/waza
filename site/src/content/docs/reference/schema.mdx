...
config:
  ...
  sandbox: # Optional; schemaVersion 1.3+
    enabled: true
...
Enables Copilot CLI's native OS sandbox for model-visible tools. Waza derives
filesystem access from the fresh task workspace and resolved
`skill_directories`. Declared skills remain read-only and are loaded through
Copilot's native skill mechanism. Optional path lists are reserved for narrow
host prerequisites that cannot be materialised in the workspace.
...
All optional capabilities default to `false`. Sandbox bypass requests are
always rejected because Waza evaluations are non-interactive. Omitting the
block or setting `enabled: false` sends no sandbox policy RPC. `readonly_paths`
and `readwrite_paths` default to empty and accept absolute paths, `~`, and set
environment-variable expansion for explicit host prerequisites. Paths must exist,
are canonicalised, cannot overlap the system temp root, and cannot receive
conflicting access grants. Sandboxed Copilot CLI processes receive an explicit
allowlisted environment, with credential-bearing proxy URLs omitted; GitHub
authentication is passed explicitly through the SDK or loaded from persisted
Copilot login. Omitting or disabling the sandbox retains the existing process
environment. Copilot applies the OS sandbox to model-visible shell commands and
local MCP/LSP subprocesses; its built-in file tools enforce the policy in-process
on a best-effort basis. Model-backed prompt graders retain the task sandbox.
Remote MCP servers and trusted program graders remain separate trust boundaries;
