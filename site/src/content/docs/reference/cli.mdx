...
Compare per-skill `SKILL.md` token deltas against a base ref.
...
Without an argument, `waza tokens diff` compares against `origin/main` by default and
falls back to `main` if `origin/main` is unavailable.

**Flags:** `--format table|json`, `--threshold <percent>`
