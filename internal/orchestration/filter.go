...
func FilterTestCases(testCases []*ExecutableTestCase, taskPatterns []string, tagPatterns []string) ([]*ExecutableTestCase, error) {
	if len(taskPatterns) == 0 && len(tagPatterns) == 0 {
		return testCases, nil
	}

	var matched []*ExecutableTestCase
...
func matchesTaskOrDisplayName(tc *ExecutableTestCase, patterns []string) (bool, error) {
	if len(patterns) == 0 {
		return true, nil
	}
...
func matchesTags(tc *ExecutableTestCase, patterns []string) (bool, error) {
	if len(patterns) == 0 {
		return true, nil
	}
